/*
 ...
 2.1 컴파일시 ... 실행파일 생성 안돼요 (무조건 해결)
 2.2 런타임시 (실행도중) 에 문제가 발생 >> 강제 종료 >> 문제가 생긴 시점에 강제종료 >> 이후 나머지 코드가 실행이 안된다 
 
 예외처리 : 프로그램이 안정적으로 종료 >> 나면 원인 분석 >> 코드 수정 ... >> 다시 배포
 
 try {
 		>> 문제가 의심되는 코드
 		>> 문제가 발생되면 >> 어떤 일이 일어날까 >> 예외가 발생한 정보를 담을 수 있는 객체 자동생성 
 		ex) 
	...
	public static void main(String[] args) {
		...
		*/
		...
		try {
			...
			e.printStackTrace(); // 에러 코드 추적하는 기능 
			/*
			java.lang.ArithmeticException: / by zero
				at Ex01_Exception.main(Ex01_Exception.java:45) 
			 */
		} 
		System.out.println("main end");
